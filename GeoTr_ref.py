...
from extractor_ref import BasicEncoderRef
from position_encoding_ref import build_position_encoding_ref
...
class TransDecoderRef(nn.Module):
    def __init__(self, num_attn_layers, hidden_dim=128):
        super(TransDecoderRef, self).__init__()
        attn_layer = attnLayerRef(hidden_dim)
        self.layers = _get_clones(attn_layer, num_attn_layers)
        self.position_embedding = build_position_encoding_ref(hidden_dim)
...
class TransEncoderRef(nn.Module):
    def __init__(self, num_attn_layers, hidden_dim=128):
        super(TransEncoderRef, self).__init__()
        attn_layer = attnLayerRef(hidden_dim)
        self.layers = _get_clones(attn_layer, num_attn_layers)
        self.position_embedding = build_position_encoding_ref(hidden_dim)
...
class FlowHeadRef(nn.Module):
    def __init__(self, input_dim=128, hidden_dim=256):
        super(FlowHeadRef, self).__init__()
...
class UpdateBlockRef(nn.Module):
    def __init__(self, hidden_dim=128):
        super(UpdateBlockRef, self).__init__()
        self.flow_head = FlowHeadRef(hidden_dim, hidden_dim=256)
...
class OverlapPatchEmbedRef(nn.Module):
...
class GeoTrRef(nn.Module):
    def __init__(self):
        super(GeoTrRef, self).__init__()

        self.hidden_dim = hdim = 256

        self.fnet = BasicEncoderRef(output_dim=hdim, norm_fn="instance")

        self.encoder_block = ["encoder_block" + str(i) for i in range(3)]
        for i in self.encoder_block:
            self.__setattr__(i, TransEncoderRef(2, hidden_dim=hdim))
        ...
        for i in self.decoder_block:
            self.__setattr__(i, TransDecoderRef(2, hidden_dim=hdim))
        ...
        self.update_block = UpdateBlockRef(self.hidden_dim)
