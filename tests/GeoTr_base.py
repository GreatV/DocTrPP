...
from extractor_base import BasicEncoderBase
from position_encoding_base import build_position_encoding_base
from torch import Tensor, nn


class attnLayerBase(nn.Module):
    def __init__(
        self,
        d_model,
        nhead=8,
...
class TransDecoderBase(nn.Module):
    def __init__(self, num_attn_layers, hidden_dim=128):
        super(TransDecoderBase, self).__init__()
        attn_layer = attnLayerBase(hidden_dim)
        self.layers = _get_clones(attn_layer, num_attn_layers)
        self.position_embedding = build_position_encoding_base(hidden_dim)
...
class TransEncoderBase(nn.Module):
    def __init__(self, num_attn_layers, hidden_dim=128):
        super(TransEncoderBase, self).__init__()
        attn_layer = attnLayerBase(hidden_dim)
        self.layers = _get_clones(attn_layer, num_attn_layers)
        self.position_embedding = build_position_encoding_base(hidden_dim)
...
class FlowHeadBase(nn.Module):
    def __init__(self, input_dim=128, hidden_dim=256):
        super(FlowHeadBase, self).__init__()
...
class UpdateBlockBase(nn.Module):
    def __init__(self, hidden_dim=128):
        super(UpdateBlockBase, self).__init__()
        self.flow_head = FlowHeadBase(hidden_dim, hidden_dim=256)
...
def trunc_normal_(x, mean=0.0, std=1.0):
    ...
    # Get values from a uniform distribution
    size = x.shape
    tmp = x.new_empty(size + (4,)).uniform_()
    # Transform to a normal distribution with specified mean/std
    tmp = mean + std * tmp
    # Clamp to ensure it's in the interval (mean - 2 * std, mean + 2 * std)
    valid = (tmp < mean + 2 * std) & (tmp > mean - 2 * std)
    ind = valid.max(-1, keepdim=True)[1]
    x.data.copy_(tmp.gather(-1, ind).squeeze(-1))


class OverlapPatchEmbedBase(nn.Module):
...
class GeoTrBase(nn.Module):
    def __init__(self):
        super(GeoTrBase, self).__init__()
        ...
        for i in self.encoder_block:
            self.__setattr__(i, TransEncoderBase(2, hidden_dim=hdim))
        ...
        for i in self.decoder_block:
            self.__setattr__(i, TransDecoderBase(2, hidden_dim=hdim))
        ...
        self.update_block = UpdateBlockBase(self.hidden_dim)
