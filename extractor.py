...
class ResidualBlock(nn.Layer):
    def __init__(self, in_planes, planes, norm_fn="group", stride=1):
        ...
        if norm_fn == "group":
            ...
            self.norm1 = nn.BatchNorm2D(planes, use_global_stats=True)
            self.norm2 = nn.BatchNorm2D(planes, use_global_stats=True)
            if not stride == 1:
                self.norm3 = nn.BatchNorm2D(planes, use_global_stats=True)
...
class BasicEncoder(nn.Layer):
    def __init__(self, output_dim=128, norm_fn="batch"):
        ...
        if self.norm_fn == "group":
            ...
            self.norm1 = nn.BatchNorm2D(64, use_global_stats=True)
